...
import { PASSWORD, USERNAME, CLUSTER, ACCESS_TOKEN } from "./config/sensitiveData.js";
...
app.get("/api/movies", (req, res, next) => {
  ...
  req.url = `${baseUrl}/${category}?language=en-US&page=1`;
  next();
}, moviesMiddleware, (req, res) => {
  res.json(req.movies);
});
...
app.get("/api/searchmovie/:search", async (req, res) => {
  try {
    ...
    console.log(search);
    const url = `${baseUrl}?query=${search}&include_adult=true&language=en-US&page=1`;
...
app.get("/api/user/:userId/watchlist", verifyToken, validateUser, async (req, res) => {
